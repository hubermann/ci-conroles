...
public function index(){
	...
	$total_pages = ceil($this->permiso->count_rows() / $per_page);
	...
	if ($total_pages > 1){ 
		for ($i=1;$i<=$total_pages;$i++){ 
			if ($page == $i) 
				...
				$data['pagination_links']  .= '<li><a href="'.base_url().'control/permisos/'.$i.'" > '. $i .'</a></li>'; 
		} 
	}
	//End Pagination
	...
	$data['query'] = $this->permiso->get_records($per_page,$start);
...
public function create(){
	...
	if ($this->form_validation->run() === FALSE){
		...
		$newpermiso = array( 'status' => $this->input->post('status'), 
...
public function delete(){
	...
	if ($this->form_validation->run() === FALSE){
		...
		$this->permiso->delete_record($id_item);
		redirect('control/permisos', 'refresh');
